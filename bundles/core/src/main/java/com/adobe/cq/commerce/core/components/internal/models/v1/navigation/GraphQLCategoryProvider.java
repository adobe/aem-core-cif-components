...
class GraphQLCategoryProvider {
    ...
    GraphQLCategoryProvider(Page page) {
        this.graphqlClient = page.getContentResource().adaptTo(GraphqlClient.class);
        if (graphqlClient == null) {
            LOGGER.warn("GraphQL client not available for page: " + page.getPath());
        }
    }

    List<CategoryTree> getChildCategories(Integer categoryId) {
        if (graphqlClient == null || categoryId == null) {
        ...
        GraphqlResponse<Query, Error> response = graphqlClient.execute(new GraphqlRequest(queryString), Query.class, Error.class, QueryDeserializer.getGson());
