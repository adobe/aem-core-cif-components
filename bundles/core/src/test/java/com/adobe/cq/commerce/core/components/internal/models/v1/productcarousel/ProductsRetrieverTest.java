...
public class ProductsRetrieverTest {
    ...
    public void testExtendedProductQuery() {
        ...
        String expectedQuery = "{products(filter:{sku:{in:[\"sku-a\",\"sku-b\"]}}){items{__typename,sku,name,thumbnail{label,url},url_key,price{regularPrice{amount{currency,value}}},... on ConfigurableProduct{variants{product{sku,name,thumbnail{label,url},price{regularPrice{amount{currency,value}}}}}},created_at,is_returnable_custom_:is_returnable}}}";
    ...
    public void testExtendedVariantQuery() {
        ...
        String expectedQuery = "{products(filter:{sku:{in:[\"sku-a\",\"sku-b\"]}}){items{__typename,sku,name,thumbnail{label,url},url_key,price{regularPrice{amount{currency,value}}},... on ConfigurableProduct{variants{product{sku,name,thumbnail{label,url},price{regularPrice{amount{currency,value}}},weight,volume_custom_:volume}}}}}}";
        Assert.assertEquals(expectedQuery, captor.getValue());
    }
