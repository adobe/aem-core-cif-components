...
public class ProductRetrieverTest {
    ...
    public void testExtendedProductQuery() {
        ...
        String expectedQuery = "{products(filter:{url_key:{}}){items{__typename,sku,name,description{html},image{label,url},thumbnail{label,url},url_key,stock_status,price{regularPrice{amount{currency,value}}},media_gallery_entries{disabled,file,label,position,media_type},... on ConfigurableProduct{configurable_options{label,attribute_code,values{value_index,label}},variants{attributes{code,value_index},product{sku,name,description{html},image{label,url},thumbnail{label,url},url_key,stock_status,color,price{regularPrice{amount{currency,value}}},media_gallery_entries{disabled,file,label,position,media_type}}}},created_at,is_returnable_custom_:is_returnable}},storeConfig{secure_base_media_url}}";
    ...
    public void testExtendedVariantQuery() {
        ...
        String expectedQuery = "{products(filter:{url_key:{}}){items{__typename,sku,name,description{html},image{label,url},thumbnail{label,url},url_key,stock_status,price{regularPrice{amount{currency,value}}},media_gallery_entries{disabled,file,label,position,media_type},... on ConfigurableProduct{configurable_options{label,attribute_code,values{value_index,label}},variants{attributes{code,value_index},product{sku,name,description{html},image{label,url},thumbnail{label,url},url_key,stock_status,color,price{regularPrice{amount{currency,value}}},media_gallery_entries{disabled,file,label,position,media_type},weight,volume_custom_:volume}}}}},storeConfig{secure_base_media_url}}";
        Assert.assertEquals(expectedQuery, captor.getValue());
    }
