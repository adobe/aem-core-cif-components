...
Ran a `--level high` code review (4 parallel finder agents, 1-vote verify) across all cart-tool changes (this plan + the configurable-cart follow-up). Findings applied here:

- **Deduplicated the cart field-selection block.** `.id().totalQuantity().items(...).prices(...)` was independently inlined in `ViewCartTool`, `AddToCartTool`, `ClearCartTool`, plus a fourth local copy in `UpdateCartItemTool` — four places that had to be kept in sync by hand. Extracted to `CartMutationClient.cartFields()` (a `public static` method, since it's needed both for a plain `Query.cart(...)` call and for a mutation's `cart(...)` output selection, which take the same `CartQueryDefinition` type). All four tools now call the one shared method.
- **Added a null-guard for `ctx.getClient()` in `CartMutationClient.execute()`.** `StoreContext.getClient()` can be `null` if commerce config resolution fails for the request (`StoreContextResolver` doesn't null-check it either) — previously this would surface as a raw `NullPointerException` from `ctx.getClient().getHttpHeaderMap()` instead of a clean, actionable error. Now throws the same `IllegalStateException` style as the existing GraphQL-client-not-resolved check.
- **Forced `HttpMethod.POST` in `ViewCartTool.fetch()`** instead of the cacheable default `execute(query)`. Not an active bug (no OSGi cache config in this repo currently caches the `mcp`-selector resource type), but cart contents mutate on every write and `CartMutationClient` has no cache-invalidation hook, so this is cheap insurance against a future config change silently serving stale cart reads.
- **One suspected bug was verified as a false positive and rejected**, not "fixed": a reviewer flagged `DtoMapper.cart()`'s `ObjectNode.put("quantity", item.getQuantity())` as an unboxing-NPE risk since `getQuantity()` returns a boxed `Double`. Verified empirically (compiled and ran a standalone test) that Java's overload resolution picks the null-safe `ObjectNode.put(String, Double)` overload at compile time based on the static `Double` type — no unboxing occurs, and the code already writes `null` safely. No change needed.
- **Deliberately not "fixed":** a reviewer noted `CartMutationClient.resolveGraphqlClient()` reproduces `MagentoGraphqlClientImpl`'s private resolution logic from `bundles/core`, and that a small public generic-execute method on `MagentoGraphqlClient` would be the more robust long-term fix. This is the same trade-off already made explicitly earlier in this phase (see the "Core change needed" decision above) — the team chose to keep the workaround inside `bundles/mcp` rather than touch `bundles/core`. Recorded here as a known, accepted risk: if `bundles/core` changes how it resolves `GraphqlClient` from a resource (e.g. stops using `ValueMapResource`), cart mutation tools will break at runtime with no compile-time signal from `bundles/core`'s own build.
- A test-coverage gap was found and documented rather than filled: `CartMutationClientTest` cannot exercise the `ComponentsConfiguration`→`ValueMapResource`→`GraphqlClient` fallback path (the one actually used in production) with plain Mockito, since sling-mock's `AemContext.registerAdapter` matches by type, not instance, and can't distinguish "the original resource" from "the synthetic wrapper resource built inside the method under test." That path remains validated by live testing only (see above) — noted directly in the test file's class-level comment so this isn't mistaken for an oversight later.

Full `bundles/mcp` green gate (`mvn -pl bundles/mcp clean install`) passed after all fixes: 57/57 tests, formatter/macker/rat clean.
