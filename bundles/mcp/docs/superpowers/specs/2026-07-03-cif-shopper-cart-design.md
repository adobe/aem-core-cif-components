...
| Execution layer | Split by operation type. `view_cart` is a **query** (`Query.cart(cartId, ...)`) so it goes through `MagentoGraphqlClient.execute(...)` exactly like every existing read tool. `add_to_cart`, `update_cart_item`, `clear_cart` are **mutations**; they cannot use `MagentoGraphqlClient.execute(...)` at all — verified via `javap` that its implementation hardcodes response deserialization to `Query.class` via `QueryDeserializer` (`MagentoGraphqlClientImpl.execute()`, `bundles/core`). Magento ships a separate `MutationDeserializer` (`com.adobe.cq.commerce.magento.graphql.gson.MutationDeserializer`, same `magento-graphql` jar) for `Mutation`-shaped responses. Mutating tools instead: (1) adapt the endpoint's own `Resource` to the raw `GraphqlClient` — `ctx.getResource().adaptTo(GraphqlClient.class)`, the same adaptation `MagentoGraphqlClientImpl` performs internally; (2) build `RequestOptions` with `.withGson(MutationDeserializer.getGson())`, headers copied from `ctx.getClient().getHttpHeaderMap()` (preserves store-context headers), and `HttpMethod.POST`; (3) call `graphqlClient.execute(new GraphqlRequest(mutationString), Mutation.class, Error.class, options)` directly. A small shared helper in `internal/tools/` wraps steps 1–3 for the three mutating tools. Mutation query strings are built the same way existing code builds query strings — `Operations.mutation(...)` mirrors `Operations.query(...)`, already used throughout `bundles/core` (e.g. `SearchResultsServiceImpl`). | No `Abstract*Retriever`-equivalent exists for mutations (none of the existing reuse abstractions cover writes). Building one in `bundles/core` for a single consumer (MCP) would be premature abstraction — nothing else in this codebase does server-side cart operations; the real storefront's React/Peregrine cart talks to GraphQL directly. All new code stays inside `bundles/mcp`. |
...
- `MagentoGraphqlClientImpl.execute(String)` (`bundles/core`) calls `graphqlClient.execute(graphqlRequest, Query.class, Error.class, options)` — hardcoded to `Query.class`, confirming it cannot deserialize a `Mutation`-shaped response.
- `com.adobe.cq.commerce.magento.graphql.gson.MutationDeserializer` exists alongside `QueryDeserializer` in the same jar/package, with the same `public static Gson getGson()` shape — the mutation-side counterpart, used by mutating cart tools per §2.
- `com.adobe.cq.commerce.magento.graphql.Operations` exposes both `query(QueryQueryDefinition)` and `mutation(MutationQueryDefinition)` static builders returning a query-string-producing object (`.toString()`), mirroring the `Operations.query(...)` pattern already used throughout `bundles/core` (e.g. `SearchResultsServiceImpl.java:451`).
- `QueryQuery.cart(String, CartQueryDefinition)` confirms fetching a cart by id is a **Query** field, not a Mutation field — so `view_cart` needs none of the above and follows the same `MagentoGraphqlClient.execute()` pattern as every existing read tool.
