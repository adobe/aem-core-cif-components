...
class CategoryCarousel {
    constructor(rootElement) {
        this._cardsContainer = rootElement.querySelector(CategoryCarousel.selectors.container);
        ...
        this._delay = 0;
        this._effect = 'linear';
        this._carousel_root = rootElement.querySelector(CategoryCarousel.selectors.root);
        this._currentRootWidth = 0;
        this._carousel_parent = rootElement.querySelector(CategoryCarousel.selectors.parent);
        this._cards = this._cardsContainer.querySelectorAll(CategoryCarousel.selectors.card);
        this._btnPrev = rootElement.querySelector(CategoryCarousel.selectors.btnPrev);
        this._btnNext = rootElement.querySelector(CategoryCarousel.selectors.btnNext);
        this._currentPos = 0;
...
CategoryCarousel.selectors = {
    self: '.cmp-categorylist',
    btnPrev: "[data-carousel-action='prev']",
    btnNext: "[data-carousel-action='next']",
    container: '.cmp-categorylist__content',
    root: '.cmp-categorylist__cardsroot',
    parent: '.cmp-categorylist__parent',
    card: '.cmp-categorylist__card'
};

(function() {
    function onDocumentReady() {
        const carouselCmp = document.querySelectorAll(CategoryCarousel.selectors.self);
        if (carouselCmp) {
            carouselCmp.forEach(function(element) {
                new CategoryCarousel(element);
            });
        }
    }
