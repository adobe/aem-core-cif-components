...
1. `commerceStaging` - when `true` (default), the `staged` field is added to the product and variant queries to drive the author-only "Staged" badge. Set to `false` for Magento Open Source backends, which do not support the `staged` field.
